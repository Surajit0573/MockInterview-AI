...
export default function ProfilePage() {
  ...
  <div className="bg-gray-800 rounded-lg p-6">
    ...
    <div className="space-y-4">
      ...
      >
        {techtags.map((tag) => (
          <option key={tag.value} value={tag.value} className="bg-gray-800 text-white">
            {tag.label}
          </option>
        ))}
      </select>
      ...
      <div className="flex flex-wrap gap-2">
        {watch("Skills", []).map((skill) => (
          <span
            key={skill}
            className="bg-gray-700 text-white px-3 py-1 text-sm flex items-center gap-1 rounded"
          >
            {skill}
            <button
              type="button"
              ...
              {[1, 2, 3, 4, 5].map((level) => (
                <div
                  ...
                  className={`w-6 h-6 rounded-full mx-1 cursor-pointer ${
                    level <= (index + 2) ? "bg-purple-600" : "bg-gray-600"
                  }`}
                />
              ))}
            </div>
